...
const TalentCard = ({
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-1 mb-0.5">
            ...
            <span className={`inline-flex items-center gap-0.5 text-[10px] font-bold px-1.5 py-0.5 rounded border ${getRatingBadgeClass(overallRating)}`}>
          ...
          <div className="flex items-center gap-1.5 mt-1 text-[9px] text-slate-500">
            <span className="truncate max-w-[110px] bg-slate-100 text-slate-600 px-1 py-0.2 rounded">
              {department}
            </span>
            <span>•</span>
          ...
          {primaryInterviewer && (
            <p className="text-[9px] text-slate-500 truncate mt-1 flex items-center gap-1 font-medium">
              <UserCheck size={9} className="text-blue-600 shrink-0" />
  ...
  return (
    <div
      ...
      className="group relative bg-white border border-slate-200/90 hover:border-blue-300 rounded-2xl p-3.5 sm:p-4 shadow-xs hover:shadow-md transition-all duration-200 cursor-pointer select-none"
    >
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-start sm:items-center gap-3.5 min-w-0 flex-1">
          ...
          <div className="relative shrink-0">
            <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-2xl overflow-hidden border-2 border-white shadow-sm bg-slate-100 ring-2 ring-slate-100 flex items-center justify-center transition-transform duration-200 group-hover:scale-105">
              {avatar ? (
                <img
                  src={avatar}
                  alt={name}
              ...
              <div
                className={`h-full w-full ${avatarBg} text-white font-bold text-sm flex items-center justify-center ${
                  avatar ? 'hidden' : 'flex'
                }`}
              >
                {avatarInitials || name.slice(0, 2).toUpperCase()}
            ...
            <span
              className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full bg-emerald-500 border-2 border-white shadow-xs"
              title="Active Talent"
            />
          </div>
              ...
              {primaryInterviewer && (
                <>
                  ...
                  <span className="inline-flex items-center gap-1 text-slate-600 font-medium">
                    ...
                    <span>Interviewer: <strong className="text-slate-800">{primaryInterviewer}</strong></span>
                  </span>
                </>
              )}
            </div>
        ...
        <div className="flex items-center justify-between lg:justify-end gap-3 sm:gap-5 border-t lg:border-t-0 pt-2.5 lg:pt-0 border-slate-100 shrink-0">
          {/* Top Functional Skills preview */}
          <div className="hidden md:flex flex-col items-start lg:items-end gap-1 max-w-[220px]">
            ...
            <div className="flex items-center gap-1 flex-wrap justify-end">
              {functionalSkills.slice(0, 3).map((skill) => (
                <span
                  ...
                  className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-slate-100 text-slate-700 border border-slate-200/60"
                >
                  {skill.name.split(' ')[0]}
                </span>
              ))}
          ...
          {/* Rating Badge */}
          <div className="flex flex-col items-center justify-center px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200/80 group-hover:border-blue-200 transition-colors min-w-[80px]">
          ...
          <div className="flex items-center">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-600 hover:text-white font-medium text-[11px] transition-all duration-200 shadow-xs group-hover:bg-blue-600 group-hover:text-white"
            >
              <Eye size={13} />
