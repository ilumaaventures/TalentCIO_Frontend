...
import {
    FileText, CheckCircle2, Plus, Search, Users,
    Loader, RefreshCw, Trash2, ExternalLink, FileStack
} from 'lucide-react';
...
import {
    ...
    acknowledgeDocument,
    deleteDocument
} from '@/features/ess-documents/api/essDocumentApi';
import UploadDocumentModal from '@/features/ess-documents/components/UploadDocumentModal';
import AcknowledgementStatusModal from '@/features/ess-documents/components/AcknowledgementStatusModal';
...
const DocumentCard = ({ doc, isAdmin, onAcknowledge, onViewReport, onDelete }) => {
    ...
    return (
        <div className={`rounded-2xl border bg-white shadow-xs transition-all hover:shadow-md ${doc.requiresAcknowledgement && !doc.viewerAcknowledged && !isAdmin ? 'border-amber-200 ring-1 ring-amber-100' : 'border-slate-100'}`}>
            <div className="p-5">
                {/* Header row */}
                <div className="flex items-start justify-between gap-3">
                    ...
                    {isAdmin && doc.requiresAcknowledgement && (
                        <button
                            ...
                            className="flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-600 hover:bg-blue-50 hover:border-blue-200 hover:text-blue-700 transition-colors shrink-0 cursor-pointer"
                        >
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {!isAdmin && doc.requiresAcknowledgement && !doc.viewerAcknowledged && (
                            <button
                                ...
                                onClick={handleAck}
                                disabled={acking}
                                className="flex items-center gap-1.5 rounded-xl bg-green-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-green-700 disabled:opacity-60 transition-colors cursor-pointer"
                            >
                        ...
                        {isAdmin && (
                            <button
                                ...
                                disabled={deleting}
                                className="flex items-center gap-1 rounded-xl border border-slate-200 p-1.5 text-slate-400 hover:bg-red-50 hover:border-red-200 hover:text-red-500 transition-colors cursor-pointer"
...
export const CompanyDocumentsTab = ({ isCurrentUserAdmin: propIsAdmin }) => {
    const { user } = useAuth();
    const isAdmin = propIsAdmin ?? (
        user?.roles?.some(r => ['Admin', 'HR Admin', 'System Admin'].includes(typeof r === 'string' ? r : r?.name))
        || user?.permissions?.includes('ess_document.manage')
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
                <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                        Official policies, forms, and circulars from HR & Management
                    </p>
                </div>
                <div className="flex items-center gap-2">
                    <button
                        type="button"
                        onClick={load}
                        className="rounded-xl border border-slate-200 bg-white p-2 text-slate-500 hover:bg-slate-50 transition-colors cursor-pointer"
                        title="Refresh"
                    >
                        <RefreshCw size={16} />
                    </button>
                    {isAdmin && (
                        <button
                            ...
                            className="flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-xs hover:bg-indigo-700 transition-colors cursor-pointer"
                        >
                ...
                <div className="flex gap-1.5 flex-wrap">
                    {CATEGORIES.map(c => (
                        <button
                            ...
                            className={`rounded-xl px-3.5 py-2 text-xs font-semibold transition-all border cursor-pointer
                                ${catFilter === c
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/50 py-16 text-center">
                    ...
                    <p className="mt-1 text-sm text-slate-400">
                        {isAdmin
                            ? 'Upload your first company document to share with employees.'
                            : 'No documents have been published for you yet.'}
                    </p>
                    {isAdmin && (
                        <button
                            type="button"
                            onClick={() => setShowUpload(true)}
                            className="mt-5 flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-2 text-sm font-semibold text-white hover:bg-indigo-700 cursor-pointer"
                        >
                            <Plus size={15} /> Upload Document
                        </button>
                    )}
